...
public class EnemyController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (hitInfo.collider != null)
        {
            ...
            {
                Debug.DrawLine(transform.position, hitInfo.point, Color.red); // If hits detects anything else turn red to signify a hit.
                LineOfSight.SetPosition(1, hitInfo.point);
                LineOfSight.colorGradient = redC;
            }
            if (hitInfo.collider.CompareTag("Player"))
            { // If raycast detects player kill player game object.
        ...
        {
            Debug.DrawLine(transform.position, transform.position + transform.right * LOS, Color.green); //if doesent collider with anything then simple turn green to signify no hit.
